...
class CompanyPostgresRepository:
    ...
    def create(self, data: dict) -> dict:
        ...
        with get_cursor() as cur:
            cur.execute(
                "INSERT INTO companies (id, name, email, phone, address, website, employee_count, annual_revenue, is_active, description, created_at, updated_at) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                (
                    new_id,
                    data.get('name'),
                    data.get('email'),
    ...
    def update(self, entity_id: str, data: dict) -> dict | None:
        updatable = ('name', 'email', 'phone', 'address', 'website', 'employee_count', 'annual_revenue', 'is_active', 'description',)
