"""Audit-hook test — companies mutations write the audit trail (same transaction)."""

import uuid


def test_companies_mutations_are_audited(client, admin_headers):
    created = client.post(
        "/api/companies",
    ...
    dele = client.delete(f"/api/companies/{eid}", headers=admin_headers)
    ...
    assert all(r["entity_type"] == "companies" and r["actor"] for r in rows)
