"""Pagination test — GET /api/companies?limit=&offset= pages the list; omitting both is unchanged."""
...
def test_companies_list_pagination(client, admin_headers):
    ...
    allp = client.get("/api/companies?limit=" + str(total), headers=admin_headers)
    assert len(allp.json()) == total
    off = client.get(
        "/api/companies?limit=" + str(total) + "&offset=1", headers=admin_headers
    )
