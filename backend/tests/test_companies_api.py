...
def test_companies_crud(client, admin_headers, user_headers):
    ...
    r = client.post("/api/companies", json={"name": "v1", "website": "v1", "industry": "v1", "employee_count": 7}, headers=admin_headers)
    ...
    upd = client.put(f"/api/companies/{entity_id}", json={"name": "n2", "website": "v2"}, headers=admin_headers)
